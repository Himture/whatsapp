...
- **Encryption at rest** — `accessToken` and `appSecret` are encrypted via AES-256-GCM. Server-side: Node `crypto` with scrypt-derived key from `ENCRYPTION_KEY`. Browser-side (local mode): Web Crypto with a per-install key in IndexedDB; optional passphrase wraps the key with PBKDF2 (600k iterations).
...
- **Browser-side accessToken.** In the zero-markup architecture the access token is decrypted in the browser to sign requests to Meta. A compromised browser extension with IndexedDB access can read it. Mitigation: use a System User Access Token scoped only to messaging, and the optional passphrase wrap of the keyring.
